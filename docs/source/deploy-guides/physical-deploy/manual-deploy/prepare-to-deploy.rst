...
在物理机上部署 HashData Lightning
前，你需要做一些准备工作。在开始部署之前，请阅读本文档以及\ :ref:`软硬件配置 <软硬件配置需求>`。

规划部署架构
------------

根据 :ref:`HashData Lightning 架构 <架构介绍>`\和软硬件配置要求规划部署架构，确定所需的服务器数量。确保所有的服务器都在一个安全组内，并且配置了互信。
